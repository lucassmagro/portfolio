...
describe('ThemeProvider', () => {
  ...
  it('abre claro por padrão no currículo', () => {
  ...
  it('respeita a preferência salva em localStorage', () => {
    localStorage.setItem('lsm-theme', 'dark')
    render(
      <ThemeProvider>
        <Probe />
      </ThemeProvider>,
  ...
  it('alterna o tema e persiste a escolha', async () => {
    ...
    // Começa claro; alterna para escuro e persiste.
    await userEvent.click(screen.getByText('toggle'))
    expect(screen.getByTestId('dark').textContent).toBe('true')
    expect(document.documentElement).toHaveClass('dark')
    expect(localStorage.getItem('lsm-theme')).toBe('dark')
  })
})
